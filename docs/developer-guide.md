...
- Intended as the destructive counterpart to additive `jbofs-sync.sh`.
...
Recursive `jbofs-cp.sh` intentionally follows rsync-style source semantics because they are less surprising than ad hoc rules:
...
`jbofs-rm.sh` removes all matching logical symlinks when invoked from a physical file path. This avoids ambiguous partial cleanup.

### Sync and prune are intentionally separate

- `jbofs-sync.sh` never deletes
- `jbofs-prune.sh` only removes broken logical symlinks
